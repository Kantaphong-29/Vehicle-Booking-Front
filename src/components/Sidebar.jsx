...
const ICONS = {
  ...
  collapse: 'M15 6 9 12l6 6',   // ชี้ซ้าย - ใช้ตอนเปิดอยู่ (กดเพื่อย่อ)
  expand: 'M9 6l6 6-6 6',        // ชี้ขวา - ใช้ตอนย่ออยู่ (กดเพื่อขยาย)
...
export default function Sidebar() {
  ...
  return (
    <aside className={`shrink-0 bg-navy-100 border-r border-ink-900/8 flex flex-col min-h-screen transition-all duration-200 ${
      collapsed ? 'w-16' : 'w-60'
    }`}>
      <div className={`py-5 border-b border-ink-900/8 flex items-center ${collapsed ? 'px-2 justify-center' : 'px-5 justify-between'}`}>
        {!collapsed && (
        ...
        <button
          ...
          className="shrink-0 w-7 h-7 flex items-center justify-center rounded-md text-ink-500 hover:bg-navy-50 hover:text-navy-800 transition-colors"
        >
